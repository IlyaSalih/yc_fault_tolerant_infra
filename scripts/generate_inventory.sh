...
ELASTICSEARCH_IP=$(terraform output -raw elasticsearch_internal_ip)
KIBANA_INTERNAL_IP=$(terraform output -raw kibana_internal_ip)
...
all:
  ...
  children:
    ...
    kibana:
      hosts:
        kibana-server:
          ansible_host: ${KIBANA_INTERNAL_IP}
      vars:
        ansible_ssh_common_args: >-
          -o StrictHostKeyChecking=no
          -o ProxyCommand="ssh -i ~/.ssh/coursework_ed25519 -o StrictHostKeyChecking=no -W %h:%p ubuntu@${BASTION_IP}"
...
echo "  elasticsearch (int): ${ELASTICSEARCH_IP}"
echo "  kibana (int):        ${KIBANA_INTERNAL_IP}"
echo "  kibana (pub):        ${KIBANA_PUBLIC_IP}"
